...
let apiUrl = 'http://localhost:3000/transactions/';
let transactions = [];
const getTransactions = async (apiUrl) => {
...
const findIndexTransaction = (id) => {
    return transactions.find(transaction => transaction.id === id)?.id;
};
...
// CREATE EDIT BUTTON
const createEditBtn = (transaction) => {
...
const createRemoveBtn = (transaction) => {
...
const renderTransactions = async () => {
    addTransaction();
    const response = await getTransactions(apiUrl);
